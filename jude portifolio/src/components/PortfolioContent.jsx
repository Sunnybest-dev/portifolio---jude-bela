...
export default function PortfolioContent() {
  ...
  useEffect(() => {
    const fetchItems = async () => {
      const { data } = await supabase.from('portfolio_items').select('*').eq('is_active', true).order('order_position');
      setItems(data || []);
    };
    const fetchSections = async () => {
      const { data } = await supabase.from('page_sections').select('*').eq('is_active', true).order('order_position');
      setSections(data || []);
    };
    ...
    <div className="w-full">
              ...
              {items.filter(item => item.category === 'Uncharted').map((item) => (
                <div key={item.id}>
                  ...
                  <div className="text-center mt-4">
                    <a href={item.youtube_url} target="_blank" rel="noopener noreferrer" className="text-[#e05532] hover:underline text-sm">
                      Watch Video
                    </a>
                  </div>
                </div>
      ...
      <div className="w-full sm:pt-16 px-6 sm:px-8 lg:px-12 font-serifDisplay text-[#1e1e1e]" style={getSectionStyle('portfolio', 'vox-borders')}>
        <div className="flex justify-center mb-8 sm:mb-10">
          <a href={items.filter(item => item.category === 'Vox Borders')[0]?.youtube_url || "https://www.youtube.com/watch?v=dQw4w9WgXcQ"} target="_blank" rel="noopener noreferrer" className="uppercase text-xs sm:text-sm tracking-widest border-b-2 border-red-500 pb-1 hover:border-gray-500 transition-colors">
            Watch Series
          </a>
        </div>
      ...
      <div className="w-full py-8 sm:pt-16 lg:pt-20 px-6 sm:px-8 lg:px-12 font-['Cormorant_Garamond',serif] text-[#1e1e1e]" style={getSectionStyle('portfolio', 'nyt')}>
        <div className="flex justify-center mb-8 sm:mb-10">
          <a href={items.filter(item => item.category === 'NYT')[0]?.youtube_url || "https://www.youtube.com/watch?v=dQw4w9WgXcQ"} target="_blank" rel="noopener noreferrer" className="uppercase text-xs sm:text-sm tracking-widest border-b-2 border-red-500 pb-1 hover:border-gray-500 transition-colors">
            Watch Series
          </a>
        </div>
        <div className="border-t border-black mb-10 sm:mb-12 lg:mb-16"></div>
        ...
        <div className="flex justify-center">
          {items.filter(item => item.category === 'NYT').length > 0 ? (
            ...
            <a href="https://www.youtube.com/watch?v=dQw4w9WgXcQ" target="_blank" rel="noopener noreferrer" className="relative w-full max-w-[850px]">
        ...
        <div className="text-center mb-3 sm:mb-4 pt-10">
          <a href={items.filter(item => item.category === 'NYT')[0]?.youtube_url || "https://www.youtube.com/watch?v=dQw4w9WgXcQ"} target="_blank" rel="noopener noreferrer" className="text-xs sm:text-sm tracking-widest uppercase border-b-2 border-red-500 inline-block pb-2 hover:border-gray-500 transition-colors">
            Play Piece
          </a>
        </div>
      </div>
      ...
      <section className="w-full bg-[#d8cfc2]  px-6 pb-12 sm:py-16">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="flex flex-col sm:flex-row justify-center items-center gap-4 text-white">
            ...
            <a href="https://www.youtube.com/@judebela" target="_blank" rel="noopener noreferrer" className="block bg-gray-900  px-8 sm:px-10 py-3 sm:py-4 text-center font-semibold text-xs sm:text-sm tracking-widest uppercase shadow-lg hover:bg-orange-500 transition-all duration-300 w-[90vw] max-w-6xl">
              Go To Channel
            </a>
          </div>
        </div>
